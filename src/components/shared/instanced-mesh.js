AFRAME.registerComponent('instanced-mesh', {
  ...
  attachEventListeners: function () {
    ...
    this.el.addEventListener(
      'memberModified',
      this.listeners.memberModified,
      false
    );
    this.el.addEventListener(
      'memberRemoved',
      this.listeners.memberRemoved,
      false
    );
  },
  ...
  update: function () {
    ...
    switch (this.data.positioning) {
      ...
      default:
        console.log(
          `Unexpected value for 'positioning' attribute: ${this.data.positioning}`
        );
  ...
  countTexturesToLoad: function () {
    ...
    this.texturesToLoad +=
      shader.isLoadingEnvMap || material.material.envMap ? 1 : 0;
  },
  ...
  increaseInstancedMeshCapacity: function () {
    ...
    this.meshNodes.forEach((node, index) => {
      ...
      var newMesh = new THREE.InstancedMesh(
        node.geometry,
        node.material,
        this.data.capacity
      );
  ...
  tick: function (time, timeDelta) {
    if (this.membersToRemove.length > 0) {
      ...
      for (var ii = 0; ii < this.members; ii++) {
        ...
        if (
          this.membersToRemove.includes(this.orderedMembersList[membersCursor])
        ) {
          //console.log(`Item to remove: ${this.orderedMembersList[membersCursor]} at position ${matrixCursor}`)
          if (this.debug) {
            console.log(
              `Removing member ${this.orderedMembersList[membersCursor]} at position ${membersCursor}`
            );
          }
...
AFRAME.registerComponent('instanced-mesh-member', {
  ...
  play: function () {
    ...
    if (!this.added && this.el.object3D.visible) {
      if (this.debug) {
