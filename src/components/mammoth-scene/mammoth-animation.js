AFRAME.registerComponent('mammoth-animation', {
  ...
  init: function () {
    ...
    this.mammothBaby = document.querySelector('#baby-mammoth');
    ...
    this.el.addEventListener(
      ...
      () => {
        ...
        this.mammothBabyState = { marker: 0, enabled: false, running: false };
  ...
  enterWalk: function () {
    ...
    if (this.mammothBabyState.enabled) {
      this.mammothBabyState.marker = this.movesManager.moveOnCurve(
        ...
        this.phaseConfig[this.phase].mammothBabySpeed,
        { useDeltaTime: true }
      );
    }
    ...
    if (
      ...
      setTimeout(() => {
        this.mammothBaby.setAttribute('animation-mixer', {
          clip: 'Walk_Fast',
          crossFadeDuration: 0.5,
          timeScale: 1
        });
        this.mammothBabyState.running = true;
        this.phaseConfig[this.phase].mammothBabySpeed =
          this.phaseConfig[this.phase].mammothBabySpeed > 0.06
            ? this.phaseConfig[this.phase].mammothBabySpeed
            : this.phaseConfig[this.phase].mammothBabySpeed + this.mammothSpeed;
      }, 6000);
      setTimeout(() => {
    ...
    if (
      ...
      if (this.mammothBabyState.running) {
        this.phaseConfig[this.phase].mammothBabySpeed =
          this.phaseConfig[this.phase].mammothBabySpeed > 0.06
            ? this.phaseConfig[this.phase].mammothBabySpeed
            : this.phaseConfig[this.phase].mammothBabySpeed + this.mammothSpeed;
