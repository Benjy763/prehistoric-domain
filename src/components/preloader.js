...
AFRAME.registerSystem('preloader', {
  schema: {
    type: { type: 'string', default: 'custom' }, //type of CSS framework to use - acceptable values are: 'bootstrap' or 'custom'
    id: { type: 'string', default: 'loading-infos' }, //ID of the auto injected preloader modal
    autoInject: { type: 'boolean', default: true }, //whether or not to auto-inject the preloader html into the page
    target: { type: 'selector', default: '#loading-infos' }, //the html target selector
    ...
    bar: { type: 'selector', default: '#loading-infos .progress-bar' }, //html class of progress bar in preloader - used to set the width
    label: { type: 'selector', default: '#loading-infos .progress-label' }, //html class of label in preloader - used to set the percentage
    labelText: { type: 'string', default: '{0}%' }, //loading text format {0} will be replaced with the percent progress e.g. 30%
  ...
  init: function () {
    ...
    document.querySelector('#' + assetsId).addEventListener(
      'loaded',
      function () {
        if (this.data.debug) {
  ...
  triggerProgressComplete: function () {
    if (this.data.slowLoad) {
      setTimeout(
        ...
        this.slowLoadTimePreloadFinish - 1000
      );
    } else {
      if (this.data.type === 'bootstrap')
        $(this.data.bar).addClass('progress-bar-success');
    ...
    if (this.data.autoClose && !this.data.clickToClose) {
      if (this.data.slowLoad) {
        setTimeout(
          ...
          this.slowLoadTimePreloadFinish
        );
      } else {
        this.triggerPreloadingComplete();
        this.closeModal();
  ...
  drawProgress: function (percentage) {
    ...
    if (this.data.label) {
      this.data.labelText = `${percentage}%`;
      this.data.label.innerHTML =
        percentage === 100 ? this.data.doneLabelText : this.data.labelText;
    }

    if (this.data.bar) {
      this.data.bar.setAttribute(this.data.progressValueAttr, percentage);
      this.data.bar.setAttribute(
        'style',
        this.data.barProgressStyle + ':' + percentage + '%'
      );
    }
  },
    ...
    if (!this.data.title) {
      ...
      var $modal = $(
        '' +
          '<div id="' +
          this.data.id +
          '" class="modal instructions-modal" tabindex="-1" role="dialog">' +
          ...
          '</div>' +
          '</div>' +
          (this.data.clickToClose
            ? '<button type="button" class="close-btn btn btn-default" data-dismiss="modal">Continue</button>'
            : '') +
          '</div>' +
          '</div>' +
          '</div>' +
  ...
  initBootstrapModal: function ($modal) {
    ...
    if (this.data.clickToClose) {
      ...
      if ($closeBtn.length > 0) {
        ...
        console.error(
          'No Bootstrap modal close button is set in the HTML. Please add a button with the data-dismiss="modal" attribute to use clickToClose.'
        );
      }
    }
  },
